...
u <- read.csv("C:/Users/michelle.sculley/Documents/2021 BUM ASSESS/Data/CPUEData.csv", header=TRUE)
...
cpue2 <-smooth[smooth$Fleet %in% flts[5:8],] %>%
  ...
  theme(legend.position="bottom", panel.grid.major = element_blank(),
        strip.text.y = element_text(size = 6),
        strip.background = element_rect(fill="burlywood1"))
png(paste0(plotdir,"\\CPUE_Smooth.png"),height=8,width=8,units="in",res=200)
Rmisc::multiplot(cpue1,cpue2, cols = 2)
dev.off()
##  Time series of residuals from the Loess fit.
## add residual to smooth data frame
png(paste0(plotdir,"\\CPUE_Res.png"),height=16,width=8,units="in",res=200)

smooth %>% mutate(residual = CPUE - hat) %>%
  ggplot(aes(x = year))+
  theme_bw()+
...
png(paste0(plotdir,"\\CPUE_corr.png"),height=8,width=8,units="in",res=200)
cr=cor(mat[,-1],use="pairwise.complete.obs")
dimnames(cr)=list(gsub("_"," ",names(mat)[-1]),gsub("_"," ",names(mat)[-1]))
cr[is.na(cr)]=0
corrplot(cr,diag=F,order="hclust",addrect=2,  method = "ellipse",  tl.col = 'black', tl.cex = 0.75)  +          
  theme(legend.position="bottom", panel.grid = element_blank())  
dev.off()
##  Cross correlations between indices to identify potential lags due to year-class effects.
png(paste0(plotdir,"\\CPUE_CorLaggs.png"),height=8,width=8,units="in",res=200)
