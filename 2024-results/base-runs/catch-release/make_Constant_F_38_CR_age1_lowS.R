## R Script filename = make_Constant_F_template.R
## Purpose: Extract input data and build an AGEPRO input file
## for a constant fishing mortality F=0.373 projection using YearAvg = [2020,2020]
...
## Catch-release for age-1 fish at 0.2 survival rate
for (f in 1:SSInput$Nfleets)
  {
    SSInput$Fishery_SelAtAge[f,2] <- SSInput$Fishery_SelAtAge[f,2]*0.8
  }
...
# Set the constant F=0.40 for the projection years
ConstantF <- 0.38
...
AGEPRO_INP(output.dir = "C:\\Users\\jon.brodziak\\Desktop\\2024 WCNPO MLS Rebuilding\\Build-Input-File",
                    ...
                    BootFac = BootFac,
                    SS_Out = SSInput,
                    ModelName="Constant_F_38_CR_age1_LowS",
                    ProjStart = 2021,
                    NYears = NYears,
                    MinAge = 1,
                    NSims = NSims,
