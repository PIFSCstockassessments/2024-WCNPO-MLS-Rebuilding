## R Script filename = make_Phased_F_template_MT.R
## Purpose: Extract input data and build an AGEPRO input file
## for a 2-phase fishing mortality projection using YearAvg = [2020,2020]
## Produces P(SSB rebuild in 2034) >= 0.60 at Phased-Frebuild using medium-term recruitment
## 4-Apr-2024
...
## The first phase runs from 2025 to 2027 at a fishing mortality TotalFPhase1
## The second phase runs from 2025 to 2027 at a fishing mortality TotalFPhase2
TotalFPhase1 <- 0.55
TotalFPhase2 <- 0.38
...
FleetRemovals <- t(sapply(ProportionF, function(p) rep(p*(TotalFPhase1/FReference),NYears)))
...
# times the Phase 2 catch biomass during years 2028-2034, or [8:NYears]
FleetRemovals[,8:NYears] <- t(sapply(ProportionF, function(p) rep(p*(TotalFPhase2/FReference),7)))
