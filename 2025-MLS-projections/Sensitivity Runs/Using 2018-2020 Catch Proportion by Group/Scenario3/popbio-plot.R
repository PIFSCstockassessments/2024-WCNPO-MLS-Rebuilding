...
jan1bio_plot <- ggplot(df, aes(x = x, y = y)) +
  ...
  ggtitle("Population Biomass in Scenario 3") +
  theme_bw() +
...
#  geom_line(aes(y = y), linetype = "dashed", linewidth = 1.1, color = "black") +
  ...
  geom_line(aes(y = avg_popbio), linetype = "dashed", linewidth = 1.2, color = "darkgreen") +

#  annotate("text", x = max(year) - 7, y = avg_popbio * 1.05, 
#           label = "Average 2001-2020", color = "darkgreen", fontface = "italic", size = 5, hjust = 0) +
