## R Script filename = make_2025_MLS_scenario2.R
## Purpose: Extract input data and build an AGEPRO input file
## for a 3-phase catch-based projection using YearAvg = [2018,2020]
...
## Read in the BSIA reported catch biomass by fleet group for 2021-2024
FleetGroupCatch2021_2024 <- read.csv("C:\\Users\\jon.brodziak\\Desktop\\2024 WCNPO MLS Rebuilding\\2025 MLS projections\\BSIA Catch\\2021-2024-mls-catch.csv", header=TRUE)

## Read in the Scenario 2 catch biomass by fleet group for 2025-2027
FleetGroupCatch2025_2027 <- read.csv("C:\\Users\\jon.brodziak\\Desktop\\2024 WCNPO MLS Rebuilding\\2025 MLS projections\\BSIA Catch\\scenario2-catch.csv", header=TRUE)
...
## The CMM 2024 phase runs from 2025 to 2027 at quotas of Q = (3225, 2400, 2400) mt
## Catches read from file=scenario2-catch.csv
## The second phase runs from 2028 to 2031 at a quota of Q = 2200 mt
## The third phase runs from 2032 to 2034 at a quota of Q = 2200 mt

TotalCatchPhase2 <- 2200
TotalCatchPhase3 <- 2200
...
# Set FleetRemovals to be the list of proportion of catch biomass by fleet in YearAvg 
...
# Set FleetRemovals to equal the BSIA mls catch during 2021-2024
# By fleet group read from file "2021-2024-mls-catch.csv"
FleetRemovals[,1] <- t(sapply(FleetGroupCatch2021_2024[,1], function(p) rep(p,1)))
FleetRemovals[,2] <- t(sapply(FleetGroupCatch2021_2024[,2], function(p) rep(p,1)))
FleetRemovals[,3] <- t(sapply(FleetGroupCatch2021_2024[,3], function(p) rep(p,1)))
FleetRemovals[,4] <- t(sapply(FleetGroupCatch2021_2024[,4], function(p) rep(p,1)))

# Set FleetRemovals to equal the scenario1 mls catch during 2025-2027
# By fleet group read from file "scenario1-catch.csv"
FleetRemovals[,5] <- t(sapply(FleetGroupCatch2025_2027[,1], function(p) rep(p,1)))
FleetRemovals[,6] <- t(sapply(FleetGroupCatch2025_2027[,2], function(p) rep(p,1)))
...
FleetRemovals[,8:(NYears-3)] <- t(sapply(ProportionCatch, function(p) rep(p*TotalCatchPhase2,4)))

# Do the same for Phase 3 catch during 2032-2034
FleetRemovals[,(NYears-2):NYears] <- t(sapply(ProportionCatch, function(p) rep(p*TotalCatchPhase3,3)))

# Set the Harvest list to be the 2-phase harvest strategy
# with catch flag = 1 in years 2021-2034, or [1:NYears] and harvest equal to FleetRemovals
Harvest <- list("Type"=c(rep(1,NYears)),"Harvest"=FleetRemovals)

# Set the Harvest list to have catch flag = 1 and catch biomass as the harvest
# in years 2021-2024, or [1:4]
Harvest[[1]][1:4] <- rep(1,4)
...
AGEPRO_INP(output.dir = "C:\\Users\\jon.brodziak\\Desktop\\2024 WCNPO MLS Rebuilding\\2025 MLS projections",
                    boot_file = boot_file,
                    NBoot = NBoot,
                    BootFac = BootFac,
                    SS_Out = SSInput,
                    ModelName="2025_MLS_scenario2",
                    ProjStart = 2021,
                    NYears = NYears,
                    MinAge = 1,
                    NSims = NSims,
