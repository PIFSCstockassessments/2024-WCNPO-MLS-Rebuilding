##  Script filename = Build_Input_File_test_Q_F_mix.R
...
## Here is an example with F-based harvest for the first 3 years and Quota-based harvest for the remaining NYears-3
## Set harvest type by period (Landings Quota=1, F=0)
HarvestType <- c(rep(0,3),rep(1,(NYears-3)))

## Set harvest total by period (3 periods of F=0.61, (NYears-3) periods of Landings Quota=2500.0 mt)
HarvestTotal <- c(rep(0.61,3),rep(2500.0,(NYears-3)))

## Apportion harvest totals by fleet and period
FleetRemovals <- t(sapply(SSInput$CatchbyFleet, function(p) p*HarvestTotal))

## Create Harvest list object
Harvest <- list("Type"=HarvestType,"Harvest"=FleetRemovals)
