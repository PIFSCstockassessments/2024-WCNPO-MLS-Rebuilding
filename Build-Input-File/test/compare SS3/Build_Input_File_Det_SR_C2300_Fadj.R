## R Script filename = Build_Input_File_Det_SR_C2300_Fadj.R
...
source(file.path(script.dir,"SS_to_Agepro_Det.R"))
              
##### Extract SSInput with SS_To_Agepro.R script #####
SSInput <- SS_To_Agepro_Det(model.dir=model.dir, script.dir=script.dir, endyr=2020, TimeStep="Year")
...
# Apply survival rate ot calculate age-0.5 recruitment on January 1st for annual projections (thousands of age-0.5 fish on January 1st in endyr+1)
...
Recruitment$Nobs <- 1  ## Set Nobs to be the number of (R) observations in Recruitment
...
#   filter(Yr %in% firstyr:lastyr) %>%
#   select(pred_recr)
Recruitment$Recruits <- 171.634428609226
## Save age-0 recruitment (thousands of age-0 fish on July 1st in endyr) for checking input
...
## FleetRemovals <- t(sapply(ProportionF, function(p) rep(p*(TotalF/F2020),NYears)))
