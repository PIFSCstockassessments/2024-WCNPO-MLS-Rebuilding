## R Script filename = get_m14_5yr_1ecdf.R
## Purpose: Extract recruitment model M14 setup for AGEPRO input file
## Get 5-year ECDF for 2014-2018
## 25-Mar-2024
...
firstyr <- 1999 ## Set firstyr to be the first year for the (R) data
...
## Harvest is the harvest strategy you want to use: first column is the harvest specification, then one column for each fleet for each year. Landings (catch) = 1, F-Mult (F) = 0
...
FleetRemovals[,1:4] <- t(sapply(ProportionF, function(p) rep(p*(InitTotalF/F2020),4)))
FleetRemovals[,8:NYears] <- t(sapply(ProportionCatch, function(p) rep(p*TotalCatchPhase2,7)))

# Set Harvest list to have catch in years 1:NYears
Harvest <- list("Type"=c(rep(1,NYears)),"Harvest"=FleetRemovals)

# Set Harvest list to have fishing mortality in years 1:4
