##  Script filename = Build_Input_File_test_model_5.R
...
RecruitType <- c(5) ## The ordered vector RecruitType is model 5
...
source(file.path(script.dir,"SS_to_Agepro_29-Nov-2023.R"))
...
# Set BH_var = 0 to produce deterministic BH recruitment values
# Recruitment$var <- SSInput$BH_var^2
Recruitment$var <- 0.0

## Calculate age-0.5 recruitment on January 1st for annual projections 
## (thousands of age-0.5 fish on January 1st in endyr+1) by rescaling alpha
Recruitment$alpha <- exp(-0.5*M0)*Recruitment$alpha

# BH_var is the SS3 variable SR_sigmaR
# Point to include lognormal bias adjustment exp(-0.5*SR_sigmaR_sqr)
# But there is no lognormal bias adjustment for deterministic BH recruitment
