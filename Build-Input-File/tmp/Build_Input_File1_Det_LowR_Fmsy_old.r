##  Script filename = Build_Input_File_Det_LowR_Fmsy_NoStd.R
...
NSims <- 100 ## Set number of simulations per bootstrap
...
RecruitType <- c(3) ## The ordered vector RecruitType is model 3
...
# Apply survival rate to calculate age-0.5 recruitment on January 1st for annual projections (thousands of age-0.5 fish on January 1st in endyr+1)
Recruitment$Recruits[,1] <- exp(-0.5*M0)*Recruitment$Recruits[,1]
} else if (RecruitType[1] == 3 || RecruitType[1] == 14)
{
## Special Deterministic Case of Model 3
...
Recruitment$Nobs <- 1  ## Set Nobs to one observed recruitment, the average R during 2001-2020

Recruitment$Recruits <- 224.835 ## Set the average R during 2001-2020 (age-0 recruits, units are thousands of fish)
...
## Set FRatio to be FMSY/F2020 = 0.63/0.58
FRatio <- 0.63/0.58

FleetRemovals <- t(sapply(SSInput$CatchbyFleet, function(p) rep(p*TotalF,NYears)))

Harvest <- list("Type"=c(rep(0,NYears)),"Harvest"=FleetRemovals)

## Use Special version of AGEPRO_Input_det.R for model 3 constant recruitment projections
source(file.path(script.dir,"AGEPRO_Input_Det.R"))
##source("AGEPRO_Input_Det.R")
...
AGEPRO_INP(output.dir = "C:\\Users\\jon.brodziak\\Desktop\\2024 WCNPO MLS Rebuilding\\Build-Input-File",
                    ...
                    BootFac = BootFac,
                    SS_Out = SSInput,
                    ModelName="test_Det_LowR_Fmsy_NoStd",
                    ProjStart = 2021,
                    NYears = NYears,
                    MinAge = 1,
                    NSims = NSims,
